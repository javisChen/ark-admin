<template>
  <a-form-model
    ...
    <a-form-model-item class="model-item" ref="description" label="商品相册" has-feedback>
      <k-upload :limit="20" :files="internalModel.pictures" @change="onUploadSuccess"/>
    </a-form-model-item>
  </a-form-model>
</template>
<script>

import GoodsBrandSelect from "@/views/product/brand/components/GoodsBrandSelect";
import GoodsCategoryCascader from "@/views/product/category/components/GoodsCategoryCascader";
...
export default {
  ...
  methods: {
    onUploadSuccess(fileList) {
      ...
      this.internalModel.pictures = fileList.map(item => item.url)
    },
